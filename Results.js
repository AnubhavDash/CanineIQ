...
// Rachel voice — soft, slightly sad, emotionally resonant
const ELEVENLABS_VOICE_ID = '21m00Tcm4TlvDq8ikWAM';

async function synthesizeSpeech(text) {
  if (!ELEVENLABS_API_KEY) throw new Error('no_key');
...
export default function Results({ data, results, onRetake, onHome }) {
  ...
  const handlePlayDogVoice = async () => {
    if (!ELEVENLABS_API_KEY) { setAudioState('no_key'); return; }
    ...
    try {
      ...
      setAudioState(e.message === 'no_key' ? 'no_key' : 'error');
    }
  };
      ...
      <div className="dog-moment">
        ...
        <div className="dog-voice-panel">
          ...
          >
            ...
            {audioState === 'no_key' && '🔑 Add ElevenLabs key in .env'}
          </button>

          {audioState === 'no_key' && (
            <p className="key-hint">REACT_APP_ELEVENLABS_API_KEY in .env</p>
          )}

          <div className="voice-note">
            Voice generated by ElevenLabs · emotionally calibrated, not diplomatic
          </div>
        </div>
      </div>
